...
const SEMESTERS = [
  { key: 'semester1', label: 'Semester 1', short: 'Sem 1' },
  { key: 'semester2', label: 'Semester 2', short: 'Sem 2' },
  { key: 'semester3', label: 'Semester 3', short: 'Sem 3' },
  { key: 'semester4', label: 'Semester 4', short: 'Sem 4' },
  { key: 'semester5', label: 'Semester 5', short: 'Sem 5' },
  { key: 'semester6', label: 'Semester 6', short: 'Sem 6' },
  { key: 'semester7', label: 'Semester 7', short: 'Sem 7' },
  { key: 'semester8', label: 'Semester 8', short: 'Sem 8' },
];
const ALL_SEMESTER_KEYS = SEMESTERS.map((s) => s.key);
...
export default function ClassroomPage() {
  ...
  const hasRead = can('classroom', 'read'), hasCreate = can('classroom', 'create'), hasUpdate = can('classroom', 'update'), hasDelete = can('classroom', 'delete');
  ...
  const [editForm, setEditForm] = useState({ name: '', code: '', capacity: '', status: 'active', semester: 'semester1', departmentId: '', courseId: '', facultyId: '' });
  ...
  const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, title: '', message: '', onConfirm: null, confirmClass: '', confirmLabel: '' });
  ...
  const [batchAttendanceSummaries, setBatchAttendanceSummaries] = useState({});
  const [studentAttendanceData, setStudentAttendanceData] = useState([]);
  const [reportViewTab, setReportViewTab] = useState('classroom');
  const [absenteesByDate, setAbsenteesByDate] = useState({});
  ...
  const totalClassrooms = useMemo(() => allClassrooms.length, [allClassrooms]);
  ...
  const filteredAbsenteesByDate = useMemo(() => {
    if (!absenteesByDate || !Object.keys(absenteesByDate).length) return {};
  ...
  const toggleDate = useCallback((d) => setExpandedDate((prev) => prev === d ? null : d), []);

  const saveAllRemarks = useCallback(async (isoDate) => {
...
{showBatchCalendar && (
  ...
  />
      ...
      <style jsx>{`
        ...
        .btn-slate{display:inline-flex;align-items:center;gap:.375rem;padding:.5rem .75rem;background:#475569;color:#fff;font-size:.75rem;font-weight:600;border-radius:.5rem;transition:all .2s}.btn-slate:hover{background:#334155}
        .btn-green{display:inline-flex;align-items:center;gap:.375rem;padding:.5rem .75rem;background:#16a34a;color:#fff;font-size:.75rem;font-weight:600;border-radius:.5rem;transition:all .2s}.btn-green:hover{background:#15803d}
        .btn-danger{display:inline-flex;align-items:center;gap:.375rem;padding:.5rem .75rem;background:#ef4444;color:#fff;font-size:.75rem;font-weight:600;border-radius:.5rem;transition:all .2s}.btn-danger:hover{background:#dc2626}
        .btn-danger-outline{display:inline-flex;align-items:center;gap:.375rem;padding:.5rem .75rem;background:#fff;color:#ef4444;font-size:.75rem;font-weight:600;border-radius:.5rem;border:1px solid #fecaca;transition:all .2s}.btn-danger-outline:hover{background:#fef2f2}
        @keyframes gradient-shift{0%,100%{background-position:0% 50%}50%{background-position:100% 50%}}.animate-gradient-shift{animation:gradient-shift 4s ease infinite}
        @keyframes slide-in-left{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}}.animate-slide-in-left{animation:slide-in-left .3s ease-out}
        @keyframes scale-in{from{transform:scale(.95);opacity:0}to{transform:scale(1);opacity:1}}.animate-scale-in{animation:scale-in .2s ease-out}
        @keyframes toast-timer{from{width:100%}to{width:0%}}.animate-toast-timer{animation:toast-timer 5s linear forwards}
      `}</style>
    </div>
  );
}