...
// Time options generation (30-minute intervals from 6:00 to 19:00)
...
export default function BulkExamScheduler({
  ...
  const fetchClassrooms = async (batchId) => {
    ...
    try {
      const response = await fetch(`/api/classrooms?batchId=${batchId}&status=active`);
      if (response.ok) {
        const data = await response.json();
        let fetchedClassrooms = [];
        if (data?.classrooms) {
          fetchedClassrooms = Array.isArray(data.classrooms) ? data.classrooms : [];
        } else if (Array.isArray(data)) {
          fetchedClassrooms = data;
        }
        // Filter active classrooms
        const activeClassrooms = fetchedClassrooms.filter(
        ...
        activeClassrooms.forEach((classroom) => {
          initialSchedule[classroom.id] = {
            ...
            classroomName: classroom.name || classroom.code,
  ...
  // Fetch active courses for selected batch
  const fetchCourses = async (batchId) => {
    ...
    try {
      const response = await fetch(`/api/course-lists?batchId=${batchId}&isActive=true`);
      if (response.ok) {
        const data = await response.json();
        let fetchedCourses = [];
        if (data?.courseLists || data?.courses) {
          const courseListData = data.courseLists || data.courses || data;
          fetchedCourses = Array.isArray(courseListData) ? courseListData : [];
        }
        const activeCourses = fetchedCourses
          .filter(cl => cl.isActive !== false)
          .map(cl => ({
            id: cl.course?.id || cl.courseId,
            name: cl.course?.name || cl.courseName || 'Unknown Course',
            code: cl.course?.code || cl.courseCode || '',
            courseListId: cl.id,
            semester: cl.semester || '',
            credits: cl.course?.credits || '',
          }));
  ...
  const validateSchedule = () => {
    ...
    scheduledClassrooms.forEach((schedule) => {
      ...
      if (!schedule.startTime) {
        newErrors[`${schedule.classroomId}-startTime`] = 'Start time is required';
        isValid = false;
      }
      if (!schedule.endTime) {
      ...
      if (schedule.startTime && schedule.endTime && schedule.startTime >= schedule.endTime) {
        newErrors[`${schedule.classroomId}-endTime`] = 'End time must be after start time';
        isValid = false;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create exams for each selected course in each scheduled classroom
      const examsToCreate = [];
      selectedCourses.forEach((courseId) => {
        const course = courses.find(c => c.id === courseId);
        scheduledClassrooms.forEach((schedule) => {
          examsToCreate.push({
            name: `${examName.trim()} - ${course?.code || course?.name || 'Course'}`,
  ...
  const totalExams = scheduledCount * selectedCourses.length;
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Schedule exams across multiple classrooms with active courses
              </p>
              ...
              <div className="mt-4 space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        disabled={!selectedDepartment || batchesLoading || isFormLoading}
                      >
                        <option value="">
                      ...
                      {errors.batch && (
                        <p className="text-xs text-red-600 mt-1">{errors.batch}</p>
                      )}
                    </div>
                  ...
                  {/* Second Row - Exam Type and Exam Name */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      {errors.examType && (
                        <p className="text-xs text-red-600 mt-1">{errors.examType}</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      />
                      {errors.examName && (
                        <p className="text-xs text-red-600 mt-1">{errors.examName}</p>
                      )}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200">
                    ...
                    {coursesLoading ? (
                      <div className="flex justify-center py-4">
                        <Icons.Loader2 size={20} className="animate-spin text-emerald-600" />
                      </div>
                    ) : courses.length > 0 ? (
                          ...
                          return (
                            ...
                            >
                              {isSelected ? (
                                ...
                                <Icons.Circle className="w-3.5 h-3.5 flex-shrink-0" />
                              )}
                      ...
                      <p className="text-sm text-emerald-700 text-center py-4">
                        No active courses found for this batch
                      </p>
                    )}
                    {errors.courses && (
                      <p className="text-xs text-red-600 mt-1">{errors.courses}</p>
                    )}
                    {courses.length > 0 && (
                ...
                {hasClassrooms && (
                  <div className="bg-blue-50 p-3 rounded-lg flex flex-wrap items-center gap-3">
                    ...
                    />
                    <select
                      className="rounded-md border border-blue-300 px-3 py-1.5 text-sm bg-white"
                      onChange={(e) => applyToAll('startTime', e.target.value)}
                    >
                      <option value="">Start Time</option>
                      {TIME_OPTIONS.map((time) => (
                        <option key={`s-${time}`} value={time}>{time}</option>
                      ))}
                    </select>
                    <select
                      className="rounded-md border border-blue-300 px-3 py-1.5 text-sm bg-white"
                    ...
                    <span className="ml-auto text-sm text-blue-700">
                      {scheduledCount} of {Object.keys(examSchedule).length} classrooms scheduled
                      {selectedCourses.length > 0 && ` • ${totalExams} total exams`}
                    </span>
                  </div>
                )}
                ...
                {classroomsLoading ? (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <input
                                ...
                                value={schedule.date}
                                onChange={(e) =>
                                  handleScheduleChange(schedule.classroomId, 'date', e.target.value)
                                }
                            ...
                            <td className="px-4 py-3">
                              <select
                                ...
                                onChange={(e) =>
                                  handleScheduleChange(schedule.classroomId, 'startTime', e.target.value)
                                }
                                className={`w-full rounded-md border px-2 py-1.5 text-sm ${
                            ...
                            <td className="px-4 py-3">
                              <select
                                ...
                                onChange={(e) =>
                                  handleScheduleChange(schedule.classroomId, 'endTime', e.target.value)
                                }
                                className={`w-full rounded-md border px-2 py-1.5 text-sm ${
                              ...
                              {errors[`${schedule.classroomId}-time`] && (
                                <p className="text-xs text-red-600 mt-0.5">Time conflict</p>
                              )}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap">
                              {schedule.date && schedule.startTime && schedule.endTime ? (
                  ...
                  <div className="text-center py-12 text-gray-500">
                    No active classrooms available for this batch
                  </div>
                ) : (
                  <div className="text-center py-12 text-gray-500">
                ...
                <div className="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse gap-3 border-t pt-4">
                  <button
                    ...
                    disabled={isFormLoading || !hasClassrooms || scheduledCount === 0 || selectedCourses.length === 0}
                  ...
                  >
                    {isFormLoading ? (
                      ...
                      <>
                        ...
                        Schedule {totalExams} Exam{totalExams !== 1 ? 's' : ''}
                      </>
                    )}
                  </button>
                  <button
              ...
              >
                  ...
                  <div className="mt-4 space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewExamType((prev) => ({ ...prev, name: e.target.value }))
                        }
                      ...
                      />
                      {newExamTypeErrors.name && (
                        <p className="mt-1 text-xs text-red-600">{newExamTypeErrors.name}</p>
                      )}
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewExamType((prev) => ({ ...prev, code: e.target.value.toUpperCase() }))
                        }
                        className={`mt-1 block w-full rounded-md border px-3 py-2 focus:outline-none focus:ring-1 ${
                      ...
                      />
                      {newExamTypeErrors.code && (
                        <p className="mt-1 text-xs text-red-600">{newExamTypeErrors.code}</p>
                      )}
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewExamType((prev) => ({ ...prev, weightage: e.target.value }))
                        }
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewExamType((prev) => ({ ...prev, description: e.target.value }))
                        }
                        className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        rows="2"
                ...
                <div className="mt-5 sm:mt-6 sm:flex sm:flex-row-reverse gap-3">
                  ...
                  >
                    {addingExamType ? (
                      <>
                        <Icons.Loader2 size={16} className="animate-spin mr-2" /> Creating...
                      </>
                    ) : (
                      <>
                        <Icons.Plus size={16} className="mr-2" /> Create Exam Type
                      </>
                    )}
                  </button>
                  <button
